...
export default function Advances({ userRole, userBranchId }) {
  ...
  async function handleSaveAdvance(e) {
    ...
    const advanceData = {
      ...
      funding_source: newAdvance.funding_source, recorded_by_branch_id: newAdvance.recorded_by_branch_id || userBranchId,
  ...
  async function handleDelete(id) {
    if (!confirm('Are you sure you want to delete this advance? This cannot be undone.')) return
        ...
        <form onSubmit={handleSaveAdvance} style={{ marginBottom: '20px', padding: '15px', backgroundColor: 'white', borderRadius: '5px', border: '2px solid #9b59b6' }}>
          ...
          {newAdvance.funding_source === 'branch_cash' && (
            <>
              ...
              <textarea placeholder="e.g., Approved via phone call" value={newAdvance.owner_instruction_notes} onChange={(e) => setNewAdvance({...newAdvance, owner_instruction_notes: e.target.value})} style={{ width: '100%', padding: '10px', marginBottom: '15px', border: '1px solid #ccc', borderRadius: '5px', boxSizing: 'border-box', minHeight: '80px' }} />
            </>
          )}
